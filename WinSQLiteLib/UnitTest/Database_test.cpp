...
namespace BaseTest
{
	TEST_CLASS(DatabasAllTest)
	{
	public:

		TEST_METHOD(DatabasectorExecCreateDropExist)
		{
			remove("test.db3");
			{
		...
		TEST_METHOD(DatabasemoveConstructor)
		{
			remove("test.db3");
			{
		...
		TEST_METHOD(DatabasecreateCloseReopen)
		{
			remove("test.db3");
			{
		...
		TEST_METHOD(DatabasebusyTimeout)
		{
			{
		...
		TEST_METHOD(DatabaseexecAndGet)
		{
			// Create a new database
			SQLite::Database db(":memory:", SQLite::OPEN_READWRITE);
		...
		TEST_METHOD(DatabaseexecException)
		{
			// Create a new database
			SQLite::Database db(":memory:", SQLite::OPEN_READWRITE);
		...
		TEST_METHOD(DatabasetryExecError)
		{
			// Create a new database
			SQLite::Database db(":memory:", SQLite::OPEN_READWRITE);
			EXPECT_EQ(SQLite::OK, db.getErrorCode());
		...
		TEST_METHOD(DatabasecreateFunction)
		{
			SQLite::Database db(":memory:", SQLite::OPEN_READWRITE);
			db.exec("CREATE TABLE test (id INTEGER PRIMARY KEY, value TEXT)");
		...
		TEST_METHOD(DatabaseloadExtension)
		{
			SQLite::Database db(":memory:", SQLite::OPEN_READWRITE);
		...
		TEST_METHOD(DatabasegetHeaderInfo)
		{
			remove("test.db3");
			{
