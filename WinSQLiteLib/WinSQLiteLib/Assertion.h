 ...
 * @file    Assertion.h
 * @ingroup SQLiteCpp
 * @brief   Definition of the WIN_SQLITE_LIB_ASSERT() macro.
 *
...
/**
 * WIN_SQLITE_LIB_ASSERT WIN_SQLITE_LIB_ASSERT() is used in destructors, where exceptions shall not be thrown
 *
 * Define WIN_SQLITE_LIB_ENABLE_ASSERT_HANDLER at the project level
 * and define a SQLite::assertion_failed() assertion handler
 * to tell SQLiteC++ to use it instead of assert() when an assertion fail.
*/
#ifdef WIN_SQLITE_LIB_ENABLE_ASSERT_HANDLER
...
#define WIN_SQLITE_LIB_ASSERT(expression, message) \
...
#define WIN_SQLITE_LIB_ASSERT(expression, message)   assert(expression && message)
